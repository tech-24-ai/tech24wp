	...
	<section id="pmpro_cancel" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_section', 'pmpro_cancel' ) ); ?>">
		...
		if ( empty( $_REQUEST['confirm'] ) && isset( $_REQUEST['levelstocancel'] ) ) {
			...
			if( $_REQUEST['levelstocancel'] !== 'all') {
				...
				$cancel_memberships_text = _n( 'Yes, cancel this auto renewal', 'Yes, cancel these auto renewal', count( $old_level_ids ), 'paid-memberships-pro' );
				$keep_memberships_text = _n( 'No, keep auto renewal', 'No, keep auto renewal', count( $old_level_ids ), 'paid-memberships-pro' );
			} else {
