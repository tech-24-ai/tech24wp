...
<ul id="topic-<?php echo esc_attr( bbp_get_topic_id() ); ?>-replies" class="bs-item-list bs-forums-items bs-single-forum-list bbp-lead-topic list-view <?php echo esc_attr( $topic_reply_count < 1 ? 'topic-list-no-replies' : '' ); ?>">
	<li class="bs-item-wrap bs-header-item align-items-center no-hover-effect topic-lead">
		<div class="item flex-1">
			...
			<div class="item-meta meta-custom">
				<span class="bs-replied neww <?php echo esc_attr( $topic_reply_count < 1 ? 'bp-hide' : '' ); ?>">
					<span class="bbp-topic-freshness-author">
					<?php
					bbp_author_link(
						array(
				...
				<span class="bs-voices-wrap">
					<?php
					$views = get_post_meta($topic_id , 'views');
					if($views[0]== "") {
						$views = 0;
					} else {
						$views = $views[0];
					...
					<?php
			if(is_user_logged_in()) {
			echo do_shortcode('[bp_template_content]');
		}
			?>
				</span>
			</div>
